...
def db_init():
    with db() as c:
        c.execute("""CREATE TABLE IF NOT EXISTS movies (
            code TEXT PRIMARY KEY, file_id TEXT, title TEXT, is_premium INTEGER DEFAULT 0)""")
        ...
        c.execute("""CREATE TABLE IF NOT EXISTS pending_payments (
            user_id INTEGER PRIMARY KEY, username TEXT, full_name TEXT, submitted_at TEXT)""")
...
async def buy_premium(event, state: FSMContext):
    ...
    if premium_check(uid):
        text = f"✅ Sizda allaqachon faol premium bor!\nMuddati: <b>{premium_expires(uid)}</b>"
        if isinstance(event, CallbackQuery):
...
async def receive_receipt(msg: Message, state: FSMContext):
    uid = msg.from_user.id
    payment_add(uid, msg.from_user.username, msg.from_user.full_name)
    await state.clear()
...
async def check_payment_status(call: CallbackQuery):
    if premium_check(call.from_user.id):
        await call.message.edit_text(f"✅ Premium obunangiz faol!\nMuddati: <b>{premium_expires(call.from_user.id)}</b>")
    else:
...
async def payment_approve(call: CallbackQuery):
    ...
    try:
        ...
        pass
    # Adminga caption yangilash
    try:
        await call.message.edit_caption(
            (call.message.caption or "") + f"\n\n✅ Tasdiqlandi — muddati: {expires[:10]}"
        )
    except Exception:
        pass
...
async def lst(call: CallbackQuery):
    ...
    rows = movie_list()
    if rows:
        lines = [f"{'👑' if p else '🆓'} <code>{c}</code> — {t}" for c, t, p in rows]
        text = "📋 <b>Kinolar:</b>\n\n" + "\n".join(lines)
    else:
...
async def add_video(msg: Message, state: FSMContext):
    ...
    await msg.answer("🔢 Kino kodini kiriting:", reply_markup=kb_cancel())
...
async def add_code(msg: Message, state: FSMContext):
    ...
    code = msg.text.strip()
    if movie_get(code):
        await msg.answer(f"⚠️ <code>{code}</code> kodi band. Boshqa kod:", reply_markup=kb_cancel())
        return
...
async def add_title(msg: Message, state: FSMContext):
    ...
    await msg.answer(f"Kino turi:\n<b>{msg.text.strip()}</b> — bepulmi yoki premiummi?", reply_markup=kb_add_type())
