...
def sub_check(user_id):
    with db() as c:
        return c.execute("SELECT 1 FROM subscribers WHERE user_id=?", (user_id,)).fetchone() is not None
...
async def start(msg: Message):
    await msg.answer(
        ...
        f"Kino olish uchun kodni yuboring.\n"
        f"Misol: <code>1234</code>\n\n"
        f"Bazada: <b>{movie_count()}</b> ta kino"
    )
...
async def check_sub(call: CallbackQuery):
    ...
    if not sub_check(call.from_user.id):
        await call.answer("❌ Hali zayavka tashlamagansiz! Kanalga o'tib Join tugmasini bosing.", show_alert=True)
        return
    row = movie_get(code)
    if not row:
...
async def add_title(msg: Message, state: FSMContext):
    ...
    movie_add(data["code"], data["file_id"], msg.text.strip())
    await state.clear()
    await msg.answer(
